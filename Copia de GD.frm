...
If Not RsPd.NoMatch Then
    Do While Not RsPd.EOF
        ...
        If RsPd!Marca = m_Marca Then
            ...
            Else
                If Depende_ITOPyG Then
                    Detalle_Normal.TextMatrix(fil, 5) = c_itopg
                Else
                    Detalle_Normal.TextMatrix(fil, 5) = c_itof
                End If
...
![Observacion 1] = CbChofer.Text
![Observacion 2] = CbPatente.Text
![Observacion 3] = Obs(0).Text
![Observacion 4] = Obs(1).Text
![Observacion 5] = Obs(2).Text
...
'        If n_Copias > 0 Then
        ...
        If MsgBox("¿ IMPRIMIR GUIA DE DESPACHO ?", vbYesNo) = vbYes Then
            ...
            Case "EIFFEL"
                ' eiffel
                GD_PrintLegal_Eiffel Str(m_Numero), Mid(m_obra, 8)
            End Select
...
Dim EVMsg$
Dim ERMsg$
Dim i As Integer, ch As String, primer_caracter As Integer, suma As Integer, checksum As Integer, resto As Integer, largo As Integer
Dim jj As Integer, m_Marca_deNV As Boolean

Dim linea As Integer, registro As String, dv As Integer
Dim poscoma1 As Integer, poscoma2 As Integer, codebar As String
'Dim Nv As Integer
...
'                Debug.Print "Arr_DatoCapturado(i)"
...
'                    poscoma2 = InStr(poscoma1 + 1, registro, ",")
                    ...
                    ' el codebar debe traer 3 slash para ser valido para le programa
                    If posslash3 > posslash2 And posslash2 > posslash1 And posslash1 > 0 Then
                    
                        j = j + 1
                    
                        m_Nv = Int(Left(codebar, posslash1 - 1))
                        m_Plano = Trim(Mid(codebar, posslash1 + 1, posslash2 - posslash1 - 1))
                        m_Rev = Trim(Mid(codebar, posslash2 + 1, posslash3 - posslash2 - 1))
                        m_Marca = Trim(Mid(codebar, posslash3 + 1))
...
'
'                        '    ComboPlano.Visible = True
                            Detalle_Normal.Row = j
                            Detalle_Normal.col = 1
                            ComboPlano.Text = m_Plano

                            ' deja cantidad en uno
                            Detalle_Normal.col = 7
                            'Detalle_Normal = "1"
                            Detalle_Normal = Arr_DatoCapturado(i, 1)
...
'                Debug.Print linea; "|"; registro; "|"; dv
                ...
                ' codigo,fecha,hora
                ' XXX,YYYYMMDD,HHMMSS
''''''''''''''''                ' XXX: es el codigo de barras, code39, largo variable
                ...
                ' viene en formato "NV/PLANO/REV/MARCA"
                ...
                If poscoma1 > 0 Then
                    ...
                    Arr_DatoCapturado(contador_registros, 1) = 1
...
Case "N"
    Cr.ReportFileName = Drive_Server & Path_Rpt & "gdn_packinglist.rpt"
Case "E"
    Cr.ReportFileName = Drive_Server & Path_Rpt & "gde_packinglist.rpt"
Case "P"
    Cr.ReportFileName = Drive_Server & Path_Rpt & "gdp_packinglist.rpt"
End Select
